...
Elm.Story.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var story = _U.list([$DataModel.SinglePaneStep({header: $DataModel.HeaderPane({content: _U.list([$DataModel.Single("\n## Welcome to the ALICE O2 Primer tutorial.\nUse arrow keys to navigate.\n")
...
Elm.O2Tutorial.make = function (_elm) {
   ...
   var MoveToPage = F2(function (a,b) {    return {ctor: "MoveToPage",_0: a,_1: b};});
   var NoOp = {ctor: "NoOp"};
   var title = F2(function (address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "margin-top",_1: "6px"}]))]),
      _U.list([A2($Html.span,
              _U.list([A2($Html$Events.onClick,address,Arrows({x: -1,y: 0}))
                      ,$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "margin-right",_1: "10px"},{ctor: "_Tuple2",_0: "cursor",_1: "pointer"}]))]),
              _U.list([$Html.text($String.fromChar(_U.chr("◀")))]))
              ,$Html.text(A2($Basics._op["++"],
              "Section: ",
              A2($Basics._op["++"],$Basics.toString(model.currentStep),A2($Basics._op["++"]," / Step: ",$Basics.toString(model.currentBuildUp)))))
              ,A2($Html.span,
              _U.list([A2($Html$Events.onClick,address,Arrows({x: 1,y: 0}))
                      ,$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "margin-left",_1: "10px"},{ctor: "_Tuple2",_0: "cursor",_1: "pointer"}]))]),
              _U.list([$Html.text($String.fromChar(_U.chr("▶")))]))]));
   });
   var stepKind = F2(function (n,model) {
      return _U.eq(n,0) ? $TutorialStyles$Styles.completeStep : _U.cmp(n,
      model.currentStep) > 0 ? $TutorialStyles$Styles.incompleteStep : $TutorialStyles$Styles.completeStep;
   });
   var stepper = function (model) {
      var navigationButton = function (n) {
         ...
         _U.list([A2($Html.a,
         _U.list([$Html$Attributes.href(A2($Basics._op["++"],"#",$Basics.toString(n))),A2(stepKind,n,model)]),
         _U.list([$Html.text($Basics.toString(n))]))]));
      };
      var numberOfSlides = $Array.length(model.slides);
      return A2($Html.ol,_U.list([$TutorialStyles$Styles.stepperStyle]),A2($List.map,navigationButton,_U.range(0,numberOfSlides - 1)));
   };
   var location2action = function (uriParts) {
      var _p0 = uriParts;
      if (_p0.ctor === "::") {
            if (_p0._1.ctor === "::") {
                  var _p1 = {ctor: "_Tuple2",_0: $String.toInt(_p0._0),_1: $String.toInt(_p0._1._0)};
   ...
   var delta2update = F2(function (old,$new) {
      var same = _U.eq(old.currentStep,$new.currentStep) && _U.eq(old.currentBuildUp,$new.currentBuildUp);
      var _p3 = same;
      if (_p3 === false) {
            return $Maybe.Just($RouteHash.set(_U.list([$Basics.toString($new.currentStep),$Basics.toString($new.currentBuildUp)])));
         } else {
            return $Maybe.Nothing;
         }
   });
   var moveToPage = F3(function (page,buildup,model) {    return _U.update(model,{currentStep: page,currentBuildUp: buildup});});
   var handleArrows = F2(function (model,a) {
      ...
      var previousPageMaxBuildUp = A2($DataModel.maxSlideBuildUp,model.currentStep - 1,slides);
      var currentPageMaxBuildUp = A2($DataModel.maxSlideBuildUp,model.currentStep,slides);
      var _p4 = _U.cmp(tentativeBuildUp,0) < 0 ? {ctor: "_Tuple2",_0: model.currentStep - 1,_1: previousPageMaxBuildUp} : _U.cmp(tentativeBuildUp,
      currentPageMaxBuildUp) > 0 ? {ctor: "_Tuple2",_0: model.currentStep + 1,_1: 0} : {ctor: "_Tuple2",_0: model.currentStep,_1: tentativeBuildUp};
      var nextPage = _p4._0;
      var nextBuildUp = _p4._1;
      var page = _U.cmp(nextPage,0) < 0 ? 0 : _U.cmp(nextPage,lastPage) > -1 ? lastPage : nextPage;
      var buildup = _U.cmp(nextBuildUp,0) < 0 ? 0 : _U.cmp(nextBuildUp,10) > 0 ? 10 : nextBuildUp;
      var nextPageMaxBuildUp = A2($DataModel.maxSlideBuildUp,model.currentStep + 1,slides);
   ...
   var update = F2(function (action,model) {
      ...
      {case "NoOp": return {ctor: "_Tuple2",_0: model,_1: $Effects.none};
         case "MoveToPage": return {ctor: "_Tuple2",_0: A3(moveToPage,_p5._0,_p5._1,model),_1: $Effects.none};
         case "Arrows": return {ctor: "_Tuple2",_0: A2(handleArrows,model,_p5._0),_1: $Effects.none};
         default: if (_p5._0 === true) {
                 return {ctor: "_Tuple2",_0: A2(handleArrows,model,{x: 1,y: 0}),_1: $Effects.none};
              } else {
                 return {ctor: "_Tuple2",_0: A2(handleArrows,model,{x: 1,y: 0}),_1: $Effects.none};
              }}
   });
   var messages = $Signal.mailbox(NoOp);
   var currentPage = Elm.Native.Port.make(_elm).outboundSignal("currentPage",
   function (v) {
      return v;
   },
   A2($Signal.map,function (n) {    return 1;},messages.signal));
   var model = {currentStep: 0,currentBuildUp: 0,slides: $Array.fromList($Story.story)};
   var init = {ctor: "_Tuple2",_0: model,_1: $Effects.none};
   var Model = F3(function (a,b,c) {    return {currentStep: a,currentBuildUp: b,slides: c};});
   var render = F2(function (buildup,pane) {
      var $eval = $DataModel.evaluateBuildUp(buildup);
      var _p6 = pane;
      switch (_p6.ctor)
      {case "ShellPane": return _U.list([A2($Html.div,
                                        _U.list([$TutorialStyles$Styles.shellTabStyle]),
                                        _U.list([A2($Html.span,_U.list([$TutorialStyles$Styles.titleStyle]),_U.list([$Html.text("Welcome to Alice O2")]))]))
                                        ,A2($Html.div,
                                        _U.list([$TutorialStyles$Styles.shellStyle]),
                                        _U.list([$Markdown.toHtml(A2($Basics._op["++"],"```\n",A2($Basics._op["++"],$eval(_p6._0.content),"```")))]))]);
         case "EditorPane": var _p7 = _p6._0;
           return _U.list([A2($Html.div,
                          _U.list([$TutorialStyles$Styles.shellTabStyle]),
                          _U.list([A2($Html.span,_U.list([$TutorialStyles$Styles.titleStyle]),_U.list([$Html.text(_p7.filename)]))]))
                          ,A2($Html.div,
         ...
         default: return _U.list([$Markdown.toHtml($eval(_p6._0.content))]);}
   });
   var applyLayout = F2(function (buildup,layout) {
      var _p8 = layout;
      switch (_p8.ctor)
      {case "SinglePaneStep": var _p9 = _p8._0;
           return A2($Html.div,
           _U.list([]),
           _U.list([A2($Html.div,_U.list([$TutorialStyles$Styles.headerStyle]),A2(render,buildup,_p9.header))
                   ,A2($Html.div,
                   _U.list([$TutorialStyles$Styles.bodyStyle]),
                   _U.list([A2($Html.div,
                   _U.list([$Html$Attributes.$class("pure-g")]),
                   _U.list([A2($Html.div,_U.list([$Html$Attributes.$class("pure-u-1-24")]),_U.list([]))
                           ,A2($Html.div,_U.list([$Html$Attributes.$class("pure-u-22-24")]),A2(render,buildup,_p9.pane))
                           ,A2($Html.div,_U.list([$Html$Attributes.$class("pure-u-1-24")]),_U.list([]))]))]))]));
         case "TwoPanesStep": var _p10 = _p8._0;
           return A2($Html.div,
           _U.list([]),
           _U.list([A2($Html.div,_U.list([$TutorialStyles$Styles.headerStyle]),A2(render,buildup,_p10.header))
                   ,A2($Html.div,
                   _U.list([$TutorialStyles$Styles.bodyStyle]),
                   _U.list([A2($Html.div,
                   _U.list([$Html$Attributes.$class("pure-g"),$TutorialStyles$Styles.innerStyle]),
                   _U.list([A2($Html.div,
                           _U.list([$Html$Attributes.$class("pure-u-1-2")]),
                           _U.list([A2($Html.div,
                           _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "padding-right",_1: "15px"}]))]),
                           A2(render,buildup,_p10.leftPane))]))
                           ,A2($Html.div,
                           _U.list([$Html$Attributes.$class("pure-u-1-2")]),
                           _U.list([A2($Html.div,
                           _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "padding-left",_1: "15px"}]))]),
                           A2(render,buildup,_p10.rightPane))]))]))]))
                   ,A2($Html.div,_U.list([]),_U.list([]))]));
         default: var _p11 = _p8._0;
           return A2($Html.div,
           _U.list([]),
           _U.list([A2($Html.div,_U.list([$TutorialStyles$Styles.headerStyle]),A2(render,buildup,_p11.header))
                   ,A2($Html.div,
                   _U.list([$TutorialStyles$Styles.bodyStyle]),
                   _U.list([A2($Html.div,
                   _U.list([$Html$Attributes.$class("pure-g"),$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "text-align",_1: "center"}]))]),
                   _U.list([A2($Html.div,
                   ...
                   _U.list([A2($Html.img,_U.list([$Html$Attributes.src(_p11.image)]),_U.list([$Html.text("A test image")]))]))]))]))
                   ,A2($Html.div,_U.list([]),_U.list([]))]));}
   });
   var currentBackground = function (model) {
      var maybeBackground = A2($Array.get,model.currentStep,model.slides);
      var _p12 = maybeBackground;
      if (_p12.ctor === "Just") {
            return A2($Html.div,_U.list([]),_U.list([A2(applyLayout,model.currentBuildUp,_p12._0)]));
         } else {
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([]),
      _U.list([A2($Html.div,
              _U.list([$Html$Attributes.$class("pure-g"),$TutorialStyles$Styles.topBarStyle]),
              _U.list([A2($Html.div,_U.list([$Html$Attributes.$class("pure-u-1-2")]),_U.list([A2(title,address,model)]))
                      ,A2($Html.div,_U.list([$Html$Attributes.$class("pure-u-1-2")]),_U.list([stepper(model)]))]))
              ,A2($Html.div,_U.list([$TutorialStyles$Styles.descriptionStyle]),_U.list([currentBackground(model)]))]));
   });
   var app = $StartApp.start({init: init
                             ,update: update
                             ,view: view
                             ,inputs: _U.list([messages.signal,A2($Signal.map,Arrows,$Keyboard.arrows),A2($Signal.map,Enter,$Keyboard.enter)])});
   var main = app.html;
